...
def plot_refined_clusters(refined_clusters, nodes_df, home_node_id):
...
def custom_clustering(distance_matrix, nodes_df, num_small_clusters, num_large_clusters, factor, precision, home_node_id = 0):
    """
    ...
    for _ in range(precision):
        ...
        angle_ranges = adjust_angles(clusters, nodes_df, angle_ranges) #, factor, num_small_clusters)
...
def find_route(sub_distance_matrix, max_travel_distance, span_cost_coefficient):
    ...
    if solution:
        route_list = extract_solution_as_list(data, manager, routing, solution)
        return route_list
    else:
        print("No solution found!")
        return None

def plot_solution_clusters(route_list, nodes_df, node_mapping, home_node_id):
    """Plot the solution path and clusters with distinct color schemes and highlight the home node distinctly."""
    # Transform the solution list using the node mapping
    mapped_solution_list = [node_mapping[node] for node in route_list]
    ...
    x = [nodes_df.loc[node_id, 'x'] for node_id in mapped_solution_list]
    y = [nodes_df.loc[node_id, 'y'] for node_id in mapped_solution_list]
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertext=[
            ...
            for node_id in mapped_solution_list
        ]
    ))

    # Add the home node distinctly
    home_mapped = node_mapping[home_node_id]  # Ensure the home node ID is mapped correctly
    fig.add_trace(go.Scatter(
        x=[nodes_df.loc[home_mapped, 'x']],
        y=[nodes_df.loc[home_mapped, 'y']],
        mode='markers',
        marker=dict(symbol='x', size=12, color='red'),
        name='Home Node',
        hoverinfo='text',
        hovertext=f"Home: Node {home_mapped}"
    ))
